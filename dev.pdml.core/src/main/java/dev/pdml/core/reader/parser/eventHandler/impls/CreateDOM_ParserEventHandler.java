...
public class CreateDOM_ParserEventHandler implements ParserEventHandler<Node, Document> {
...
*/
    public void onAttributes ( @NotNull ASTNodeAttributes attributes, @NotNull Node parentNode ) throws Exception {
        ...
        for ( ASTNodeAttribute attribute : attributes.getList() ) {
            ...
            if ( ! attribute.hasNamespace() ) {
                ...
                addAttributeWithNamespace (
                    element, attribute.getNamespaceURI().toString(), attribute.getName().fullName(), value );
            }
        }
    }
...
*/
    ...
    public void onComment ( @NotNull TextToken comment, @NotNull Node parentElement ) {
        ...
        String stripped = ParserHelper.stripStartAndEndFromComment ( comment.getText() );
    ...
    private Element createElement ( ASTNodeName name ) {
        ...
        if ( ! name.hasNamespace() ) {
            ...
            return document.createElementNS(
                name.getNamespaceURIText ().toString(), name.fullName() );
        }
    }
}
